...
def greedy_algorithm():
    ...
    2. 循环直到所有客户端都被服务：
       ...
       - 对每个客户端，遍历所有可用节点（不包括客户端节点）：
         a. 检查节点安全级别是否满足客户端安全需求（必须）
         b. 检查节点容量约束（如果容量不足则跳过）
         c. 计算Sec-MSLn,u
       - 选择全局最小的Sec-MSLn,u对应的(节点,客户端)组合
       - 将选中的节点加入MS，客户端加入CS
       - 更新节点已用容量
    ...
    MS = set()  # 已选择的节点集合
    CS = set()  # 已服务的客户端集合
    # 获取实际部署的客户端（根据client节点数量动态确定）
    client_deployment = get_client_deployment()
    U = set(client_deployment.keys())  # 实际部署的客户端集合
    ...
    while CS != U:
        ...
        for client_id in U - CS:
            ...
            for node_id in available_nodes:
                ...
                # 计算Sec-MSLn,u（若进行了安全补足，使用调整后的安全开销）
                # 安全开销只计算安全补足的NF，如果没有补足则为0
                if adjusted_security_cost is None:
                    # 没有安全补足，安全开销为0
                    result = calculate_sec_msl(node_id, client_id, security_cost_override=0.0)
                else:
                    # 有安全补足，使用补足的NF的开销
                    result = calculate_sec_msl(node_id, client_id, security_cost_override=adjusted_security_cost)
                    # 添加安全补足相关的调试信息
                    result["_augmentation_plan"] = augmentation_plan
                    result["_original_node_security_level"] = node_security_level
                    result["_required_security_level"] = client_security_req
                
                if "error" in result:
                    print(f"    节点 {node_id} 计算失败: {result['error']}")
                    continue
                
                sec_msl_value = result["sec_msl"]
                print(f"    节点 {node_id}: Sec-MSL = {sec_msl_value:.4f}")
                
                # 更新最小Sec-MSL
                if sec_msl_value < min_sec_msl:
                    min_sec_msl = sec_msl_value
                    best_node = node_id
        ...
        if best_node is not None and best_client is not None:
            ...
            print(f"    Sec-MSL值: {min_sec_msl:.4f}")
