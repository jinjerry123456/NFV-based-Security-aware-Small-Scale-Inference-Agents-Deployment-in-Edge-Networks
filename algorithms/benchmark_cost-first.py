...
基于GPU容量优先的部署策略：为每个客户端选择GPU容量（gpu_unit）最低的节点提供服务
与基于Sec-MSL的贪心算法不同，此算法直接根据节点的GPU容量进行选择
选择标准：节点的gpu_unit最低（平局时选择总成本更低的节点）
总成本 = 安全开销 + 算力开销 + 带宽开销（用于统计和平局判定）
"""
...
def cost_first_algorithm():
    """
    Cost-first基准算法：为每个客户端选择安全开销/客户端数量最小的节点提供服务
    
    实验配置：
    - 只处理实际部署的客户端（数量根据client节点配置动态确定），避免资源浪费
    ...
    2. 循环直到所有客户端都被服务：
       ...
       - 对每个客户端，遍历所有可用节点（不包括客户端节点）：
         ...
         c. 计算安全开销（只计算安全补足的NF）
         d. 计算安全开销/客户端数量
       - 选择全局最小的安全开销/客户端数量对应的(节点,客户端)组合
       - 将选中的节点加入MS，客户端加入CS
       - 更新节点已用容量
    ...
    print("=" * 70)
    print("开始执行Cost-first基准算法（选择安全开销/客户端数量最小的节点）")
    print("=" * 70)
    ...
    while CS != U:
        ...
        min_security_cost_per_client = float('inf')
        best_node = None
        best_client = None
        best_details = None
        ...
        for client_id in U - CS:
            ...
            for node_id in available_nodes:
                ...
                # 计算安全开销/客户端数量（选择指标）
                security_cost_per_client = security_cost_val / client_quantity if client_quantity > 0 else float('inf')
                
                # 获取节点的GPU容量（gpu_unit）（用于显示）
                node_info = get_node_info(node_id)
                node_gpu_unit = node_info.get("gpu_unit", 0.0)
                ...
                result = {
                    ...
                    "security_cost_per_client": security_cost_per_client,
                    "gpu_unit": node_gpu_unit,
                ...
                print(f"    节点 {node_id}: 安全开销/客户端数量 = ${security_cost_per_client:.4f}, 总成本 = ${total_cost_val:.2f} (安全: ${security_cost_val:.2f}, 算力: ${compute_cost_val:.2f}, 带宽: ${bandwidth_cost_val:.2f})")
                
                # 更新最小安全开销/客户端数量
                # 当安全开销/客户端数量更小时，直接选择
                # 当安全开销/客户端数量相等时，选择总成本更低的节点（tie-breaking规则）
                if security_cost_per_client < min_security_cost_per_client:
                    # 安全开销/客户端数量更小，直接选择
                    min_security_cost_per_client = security_cost_per_client
                    best_node = node_id
                    best_client = client_id
                    best_details = result
                elif security_cost_per_client == min_security_cost_per_client:
                    # 安全开销/客户端数量相同时，选择总成本更低的节点（tie-breaking）
                    if best_details is None or total_cost_val < best_details['total_cost']:
                        ...
                        if best_details is not None:
                            print(f"      平局：选择总成本更低的节点（新总成本: ${total_cost_val:.2f} < 旧总成本: ${old_cost:.2f}）")
                        best_node = node_id
                        best_client = client_id
                        best_details = result
        ...
        if best_node is not None and best_client is not None:
            ...
            print(f"    安全开销/客户端数量: ${best_details['security_cost_per_client']:.4f}")
...
def main():
    """主函数：运行Cost-first基准算法"""

    print("开始运行Cost-first基准算法...")
    result = cost_first_algorithm()
