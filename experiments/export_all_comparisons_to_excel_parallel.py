...
# 默认参数
DEFAULT_NUM_RUNS = 1
DEFAULT_GENERATED_TOPOLOGIES_PER_COMBO = 1
...
def run_experiment_and_save_excel(args):
    ...
    try:
        ...
        from experiments.export_all_comparisons_to_excel import (
            ...
            aggregate_by_client_acceptance, write_client_acceptance_sheet,
        )
...
def main(
    ...
    print("=" * 80)
    print("并行运行六个实验并导出 Excel 汇总（每个实验独立写入Excel）")
    print("=" * 80)
    ...
    # 确定使用的进程数（现在可以使用6个进程，因为每个进程独立写入Excel）
    if num_processes is None:
        num_processes = min(cpu_count(), 6)  # 最多6个进程（对应6个实验）
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="并行运行六个实验并导出 Excel 汇总（每个实验独立写入Excel文件）")
