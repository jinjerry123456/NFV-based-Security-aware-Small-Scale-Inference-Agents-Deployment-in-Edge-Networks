# -*- coding: utf-8 -*-
"""
按节点上 NF instance 个数（NODE_DATA_2~5）对比四种算法：
1. My Algorithm (基于Sec-MSL的贪心算法)
2. Cost-first Benchmark
3. Price-first Benchmark
4. Security-first Benchmark

自变量：node_nf_count ∈ {2, 3, 4, 5}（对应 NODE_DATA_2~5）

在每个 node_nf_count 下聚合的维度：
- 节点个数: [20, 30, 40, 50]
- 拓扑度数: degree ∈ [2, 3, 4, 5]
- 客户端数量: [3, 5, 8, 10]
...
def run_all_algorithms_by_nf_instances():
    """
    以 node_nf_count ∈ {2,3,4,5} (NODE_DATA_2~5) 作为自变量：
    - 对每个 node_nf_count：
      - 遍历安全需求 security_requirement ∈ {10, 18, 26, 34}；
        ...
        dict: {
            node_nf_count: {
                ...
                node_size: {
                    degree: {
                        client_count: {
                            algorithm_name: [result_dict_or_None, ... 共 NUM_RUNS 个]
                            }
                        }
                    }
                }
    ...
    if VERBOSE_PROGRESS:
        print("=" * 80)
        print("按节点 NF instance 个数对比四个算法 (NODE_DATA_2~5)")
        print("聚合维度: security_requirement = 10/18/26/34, 节点个数, 度数, 客户端数量")
        print("=" * 80)
    ...
    for node_nf_count in NODE_NF_COUNTS:
        if VERBOSE_PROGRESS:
            print(f"\n{'=' * 80}")
            print(f"节点 NF instance 个数: {node_nf_count} (使用 NODE_DATA_{node_nf_count})")
            print(f"{'=' * 80}")

        # per_nf_results[security_requirement][node_size][degree][client_count][算法名] = [result, ... NUM_RUNS]
        per_nf_results = {}
        ...
        for security_requirement in SECURITY_REQUIREMENTS:
            ...
            # per_security_results[node_size][degree][client_count][算法名] = [result, ... NUM_RUNS]
                ...
                for client_count in CLIENT_COUNTS:
                    ...
                    for idx, (alg_name, alg_func) in enumerate(algorithm_configs, 1):
                        ...
                        for topo_instance_id, matrix in topologies.items():
                            ...
                            if VERBOSE_PROGRESS:
                                print("✓")
                        per_client_results[alg_name] = all_runs
            ...
            per_nf_results[security_requirement] = per_security_results

        all_results[node_nf_count] = per_nf_results
...
def print_summary(all_results):
    """
    打印按 node_nf_count 聚合后的算法对比摘要：
    - 自变量：node_nf_count = 2/3/4/5
    - 在每个 node_nf_count 下，对所有 security_requirement / node_size / degree / client_count 组合聚合；
    - 对每个指标输出：最小值 / 最大值 / 平均值。
    """
    print("\n" + "=" * 80)
    print("按节点 NF instance 个数的算法对比摘要")
    print("=" * 80)

    algorithm_names = ALGORITHM_NAMES
    nf_counts = sorted(all_results.keys())

    metrics = METRICS

    for node_nf_count in nf_counts:
        results_by_security = all_results[node_nf_count]

        print(f"\n{'-' * 80}")
        print(
            f"节点 NF 个数 {node_nf_count} (NODE_DATA_{node_nf_count})，"
            f"在安全需求 {SECURITY_REQUIREMENTS}、节点个数 {NODE_SIZES}、"
            f"度数 2/3/4/5 与客户端数量 {CLIENT_COUNTS} 上聚合"
        )
        print(f"{'-' * 80}")
        ...
        for metric_name, metric_id, fmt in metrics:
            for stat_name in ("最小值", "最大值", "平均值"):
                ...
                for alg_name in algorithm_names:
                    values = []
                    # 遍历所有 security_requirement / node_size / degree / client_count 组合
...
def write_nf_sheet(wb, summary):
    ...
    for nf_count in sorted(summary.keys()):
        ...
        cell.value = f"节点 NF 个数 {nf_count} (NODE_DATA_{nf_count})"
        cell.font = Font(bold=True)
        row_idx += 1

        for metric_name, metric_id, fmt in METRICS:
            for stat_name, stat_index in (("最小值", 0), ("最大值", 1), ("平均值", 2)):
                row = [nf_count, f"{metric_name}-{stat_name}"]
                for alg_name in ALGORITHM_NAMES:
                    mn, mx, avg = per_nf[alg_name].get(metric_id, (None, None, None))
...
def main():
    print("=" * 80)
    print("按节点 NF instance 个数（NODE_DATA_2~5）对比四个算法")
    print("=" * 80)

    all_results = run_all_algorithms_by_nf_instances()
    print_summary(all_results)

    # 聚合并写入 Excel，命名规则：nf_instances + GENERATED_TOPOLOGIES_PER_COMBO + GENERATED_SEED_BASE + time
    print("\n开始聚合并写入 Excel ...")
    summary = aggregate_by_nf(all_results)

    wb = Workbook()
    default_sheet = wb.active
    wb.remove(default_sheet)
    write_nf_sheet(wb, summary)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_name = (
        f"nf_instances_{NUM_RUNS}_{GENERATED_TOPOLOGIES_PER_COMBO}_"
        f"{timestamp}.xlsx"
    )
