...
def run_all_algorithms_by_degree():
    """
    以 degree=2/3/4/5 作为自变量：
    - 对每个 degree：
      - 遍历安全需求 security_requirement ∈ {10, 18, 26, 34}；
      - 遍历节点规模 size ∈ {20,30,40,50}；
    - 对每个 size 的相应 NETWORK_TOPOLOGY_size_degreeD；
    - 再遍历 client_count ∈ {3,5,8,10} 和 NODE_DATA_2~5；
    - 对四个算法运行，收集结果。
    ...
    if VERBOSE_PROGRESS:
        print("=" * 80)
        print("按拓扑度数对比四个算法 (degree=2/3/4/5)")
        print("聚合维度: security_requirement = 10/18/26/34, 节点个数, 客户端数量, NODE_DATA")
        print("=" * 80)
    ...
    for degree in degrees:
        ...
        # per_degree_results[security_requirement][node_size][client_count][算法名][node_nf_count] = [result, ... NUM_RUNS]
        per_degree_results = {}
        ...
        for security_requirement in SECURITY_REQUIREMENTS:
            ...
            # per_security_results[node_size][client_count][算法名][node_nf_count] = [result, ... NUM_RUNS]
            ...
            for node_size in NODE_SIZES:
                ...
                for client_count in CLIENT_COUNTS:
                    ...
                    per_size_results[client_count] = per_client_results

                per_security_results[node_size] = per_size_results

            per_degree_results[security_requirement] = per_security_results

        all_results[degree] = per_degree_results
...
def print_summary(all_results):
    """
    打印按 degree 聚合后的算法对比摘要：
    - 自变量：degree = 2/3/4/5
    - 在每个 degree 下，对所有 security_requirement / node_size / client_count / NODE_DATA_2~5 组合聚合；
    - 对每个指标输出：最小值 / 最大值 / 平均值。
    """
    print("\n" + "=" * 80)
    print("按拓扑度数的算法对比摘要")
    ...
    degrees = sorted(all_results.keys())

    metrics = METRICS

    for degree in degrees:
        results_by_security = all_results[degree]

        print(f"\n{'-' * 80}")
        print(
            f"度数 {degree} 的拓扑，在安全需求 {SECURITY_REQUIREMENTS}、"
            f"节点个数 {NODE_SIZES}、客户端数量 {CLIENT_COUNTS} 与 NODE_DATA_2~5 上聚合"
        )
        print(f"{'-' * 80}")
        ...
        for metric_name, metric_id, fmt in metrics:
            for stat_name in ("最小值", "最大值", "平均值"):
                ...
                for alg_name in algorithm_names:
                    values = []
                    # 遍历所有 security_requirement / node_size / client_count / node_nf_count 组合
...
def write_degree_sheet(wb, summary):
    ...
    for degree in sorted(summary.keys()):
        ...
        cell.value = f"度数 {degree}"
        cell.font = Font(bold=True)
        row_idx += 1

        for metric_name, metric_id, fmt in METRICS:
            for stat_name, stat_index in (("最小值", 0), ("最大值", 1), ("平均值", 2)):
                row = [degree, f"{metric_name}-{stat_name}"]
                for alg_name in ALGORITHM_NAMES:
                    mn, mx, avg = per_deg[alg_name].get(metric_id, (None, None, None))
...
def main():
    print("=" * 80)
    print("按拓扑度数（2/3/4/5）对比四个算法")
    print("=" * 80)

    all_results = run_all_algorithms_by_degree()
    print_summary(all_results)

    # 聚合并写入 Excel，命名规则：degree + NUM_RUNS + GENERATED_TOPOLOGIES_PER_COMBO + time
    print("\n开始聚合并写入 Excel ...")
    summary = aggregate_by_degree(all_results)

    wb = Workbook()
    default_sheet = wb.active
    wb.remove(default_sheet)
    write_degree_sheet(wb, summary)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_name = (
        f"degree_{NUM_RUNS}_{GENERATED_TOPOLOGIES_PER_COMBO}_"
        f"{timestamp}.xlsx"
    )
